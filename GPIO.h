...
typedef struct
{
	/** Port A corresponding interrupt flag*/
	uint8 flagPortA : 1;
	/** Port B corresponding interrupt flag*/
	uint8 flagPortB : 1;
	/** Port C corresponding interrupt flag*/
	uint8 flagPortC : 1;
	/** Port D corresponding interrupt flag*/
	uint8 flagPortD : 1;
	/** Port E corresponding interrupt flag*/
	uint8 flagPortE : 1;
} GPIO_interruptFlags_t;	/** Custom struct type containing the ports interrupt flags*/
...
/*! This data type is used to configure the initial state for output pins*/
typedef enum{
		GPIO_LOW,	/*!< The initial logic level is 0*/
		GPIO_HIGH	/*!< The initial logic level is 1*/
		}GPIO_OUTPUT_INITAL_LOGIC_LEVEL;

/*! */

		typedef enum{
					GPIO_INTERRUPT_DISABLED,/*!< GPIO selected port interrupt disabled*/
					GPIO_INTERRUPT_ENABLED	/*!< GPIO selected port interrupt enabled*/
}GPIO_INTERRUPTS_STATUS;
...
typedef struct{
	GPIO_portNameType GPIO_port;
	BitsType GPIO_pin;
	GPIO_pinControlRegisterType pinControlRegisterPORT_config;
	GPIO_PIN_CONFIG	GPIO_direction;
	GPIO_OUTPUT_INITAL_LOGIC_LEVEL GPIO_initialState;
	GPIO_INTERRUPTS_STATUS GPIO_interrupt;
	PriorityLevelType GPIO_priority;
} GPIO_ConfigType;
//typedef GPIO_interruptFlags_t GPIO_intrStatusFlag;
...
/********************************************************************************************/
/*!
 	\brief	This function clears the interrupt StatusFlag of a specified port

 	\param[in]	gpio The desired port to clear its interrupt StatusFlag
 	\return 0x02 in case of Error
 */
...
/********************************************************************************************/
/*!
 	 \brief This function returns the current status of the StatusFlag of an interrupt of an specified port

 	 \param[in] gpio The desired port to know its status
 	 \return The status of the interrupt flag of the wanted port
 */
